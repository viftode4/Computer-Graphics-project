...
struct Mesh {
	...
	// get centroid of triangle with index x 
	// of the curent mesh
	glm::vec3 getCentroid(const int& x) {
		return (vertices[triangles[x].x].position + vertices[triangles[x].y].position + vertices[triangles[x].z].position) / 3.0f;		
	}
