    ...
    , m_rotationEulerAngles(rotationX, rotationY, 0)
{
    ...
    lens_radius = 0;
    samples = 1;
    focus_distance = m_distanceFromLookAt;
}


// for DOF
void Trackball::setDOFradius(const float r) {
    lens_radius = r;
}

void Trackball::setDOFfocusDistance(const float r) {
    focus_distance = r;
}

float Trackball::getFocusDistance() const
{
    return focus_distance;
}

//for Antialising
void Trackball::setSamples(const int no) {
    samples = no;
}

int Trackball::getSamples() const
{
    return samples;
}
float Trackball::getLensRadius() const {
    return lens_radius;
}
bool Trackball::getAlgorithm() const
{
    return random;
}
void Trackball::setAlgorithm(const bool x) {
    random = x;
}
...
glm::vec3 Trackball::positionWithOffset(const glm::vec3& offset) const
{
    return m_lookAt + glm::quat(m_rotationEulerAngles) * (glm::vec3(0, 0, -m_distanceFromLookAt) + offset);
}
...
{
    const glm::vec3 offset = random_unit() * lens_radius;
    const glm::vec3 cameraSpaceDirection = glm::normalize(( -1.0f / getFocusDistance() * offset + glm::vec3(-pixel.x * m_halfScreenSpaceWidth, pixel.y * m_halfScreenSpaceHeight, 1.0f)));
    Ray ray;
    ray.debugColor = 0;
    ray.origin = positionWithOffset(offset);
