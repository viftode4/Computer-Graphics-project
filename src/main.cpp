...
int main(int argc, char** argv)
{
    ...
    if (!config.cliRenderingEnabled) {
        ...
        bool debugAntialiasing { false };
        int algoAntialiasing { 0 };

        bool debugDOF { false };

        std::optional<std::vector<Ray>> debugRays;
            ...
            if (action == GLFW_PRESS) {
                ...
                case GLFW_KEY_R: {
                    const auto tmp = window.getNormalizedCursorPos();
                        if (!debugAntialiasing && ! debugDOF) {
                        // Shoot a ray. Produce a ray from camera to the far plane
                        optDebugRay = camera.generateRay(tmp * 2.0f - 1.0f);
                        } else if (debugAntialiasing) {
                            std::vector<Ray> raysVector;
                            raysVector.push_back(camera.generateRay(tmp * 2.0f - 1.0f));
                                ...
                                for (int i = 0; i < camera.getSamples(); i++)
                                    for (int j = 0; j < camera.getSamples(); j++) {
                                        const glm::vec2 normalizedPixelPos {
                                            float(tmp.x + (i + random_float()) / camera.getSamples() - 0.5f) * 2.0f - 1.0f,
                                            float(tmp.y + (j + random_float()) / camera.getSamples() - 0.5f) * 2.0f - 1.0f
                                        };
                                        raysVector.push_back(camera.generateRay(normalizedPixelPos));
                                    }
                            }
                            // option algorithm 2 for the truly random 
                                else {
                                for (int i = 0; i < camera.getSamples(); i++) {
                                    const glm::vec2 normalizedPixelPos {
                                        float(tmp.x + random_float() - 0.5f) * 2.0f - 1.0f,
                                        float(tmp.y + random_float() - 0.5f) * 2.0f - 1.0f
                                    };
                                    raysVector.push_back(camera.generateRay(normalizedPixelPos));
                                }
                            }
                           
                            debugRays = raysVector;
                            raysVector.resize(0);
                            raysVector.shrink_to_fit();
                        } else if (debugDOF) {
                            std::vector<Ray> raysVector;
                            auto radius = camera.getLensRadius();
                            camera.setDOFradius(0.0f);
                            //generate straight ray
                            raysVector.push_back(camera.generateRay(tmp * 2.0f - 1.0f));
                            camera.setDOFradius(radius);

                            // generate 50 rays for the dof effect
                            for (int i = 0; i < 50; i++) {
                                raysVector.push_back(camera.generateRay(tmp * 2.0f - 1.0f));
                            }
        ...
        while (!window.shouldClose()) {
            ...
            ImGui::Begin("Final Project");
            {
                ...
                if (ImGui::Combo("Scenes", reinterpret_cast<int*>(&sceneType), items.data(), int(items.size()))) {
                    ...
                    if (!debugAntialiasing) {
                        ...
                        if (debugRays) {
                            HitInfo dummy {};
                            for (auto& it : *debugRays) {
                                bvh.intersect(it, dummy, config.features);
                            }
                        }
                    }
                }
            ...
            if (ImGui::CollapsingHeader("Extra Features")) {
                ...
                if (config.features.extra.enableDepthOfField) {
                    ...
                    auto radius = camera.getLensRadius();
                    ImGui::SliderFloat("Focal Distance", &distance, 0.05f, 10.0f);
                    ImGui::SliderFloat("Lens Radius", &radius, 0.0f, 0.6f);
                    ImGui::Checkbox("DEBUG", &debugDOF);
                    camera.setDOFradius(radius);
                    camera.setDOFfocusDistance(distance);
                } else
                    debugDOF = 0;
                ...
                if (config.features.extra.enableMultipleRaysPerPixel) {
                    auto samples = camera.getSamples();
                    ImGui::SliderInt("Samples", &samples, 1, 20);
                    camera.setSamples(samples);
                    ImGui::Checkbox("DEBUG", &debugAntialiasing);
                    ImGui::SliderInt("Algorithm", &algoAntialiasing, 0, 1);
                    camera.setAlgorithm(algoAntialiasing);
                } else
                    debugAntialiasing = 0;
            ...
            case ViewMode::Rasterization: {
                ...
                if (optDebugRay && !(debugAntialiasing || debugDOF)) {
                ...
                }else if (debugRays && (debugAntialiasing || debugDOF)) {
                    ...
                    for (auto& it : *debugRays)
                        (void)getFinalColor(scene, bvh, it, config.features);
            ...
            case ViewMode::RayTracing: {
                if (!config.features.extra.enableMultipleRaysPerPixel)
                    camera.setSamples(1);
                if (!config.features.extra.enableDepthOfField) {
                    camera.setDOFradius(0);
                    camera.setDOFfocusDistance(1.0f);
                }
